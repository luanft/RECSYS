...
class JobModel extends Model {
	...
	public function AddNewJob($AccountId, $JobName, $Location, $Salary, $Description, $Company, $Tags, $Requirement, $Benifit, $Expired, $Source, $type) {
		...
		$sql = "INSERT INTO `job`(`AccountId`, `JobName`, `Location`, `Salary`, `Description`, `Company`, `Tags`, `Requirement`, `Benifit`, `Expired`, `Source`, `Category`)" . 
		" VALUES ($AccountId,'$JobName','$Location','$Salary','$Description','$Company','$Tags','$Requirement','$Benifit','$Expired','$Source',$type)";			
		$this->connection->connect ();			
...
class XPathModel extends Model {
	...
	public function save($home_url, $base_url, $xpath_code, $login_url, $login_data, $job_xpath, $company_xpath, $location_xpath, $description_xpath, $salary_xpath, $requirement_xpath, $benifit_xpath, $expired_xpath, $tags_xpath, $category) {
		...
		$this->connection->connect ();
		$sql = "INSERT INTO `job_xpath`(`home_url`, `base_url`, `xpath_code`, `job_xpath`, `company_xpath`, `location_xpath`, `description_xpath`, `salary_xpath`, `requirement_xpath`, `benifit_xpath`, `expired_xpath`, `tags_xpath`, `login_url`, `login_data`,`category`) VALUES ('$home_url','$base_url','$xpath_code','$job_xpath','$company_xpath','$location_xpath','$description_xpath','$salary_xpath','$requirement_xpath','$benifit_xpath', '$expired_xpath', '$tags_xpath','$login_url','$login_data','$category')";
		$ret = $this->connection->write ( $sql );
		$this->connection->close ();
		return $ret;
	}
...
class CategoryModel extends Model {
	...
	public function insert($category) {
		$sql = "INSERT INTO `category`(`Description`) VALUES (' $category')";
		$this->connection->connect ();
		$this->connection->write ( $sql );
		$this->connection->close ();
	}
